...
app.post('/signup', async (req, res) => {
  const { email, password, age, phone, periodStart, periodEnd, name } = req.body;
...
app.post('/countdown', async (req, res) => {
  ...
  const existingUser = await db.collection('users').findOne({ email });
  if (existingUser) {
    start = existingUser.periodStart.split("/")[1]
    let date_ob = new Date();
    let date = ("0" + date_ob.getDate()).slice(-2)
    nextPeriod = ((30-parseInt(date)) + parseInt(start))
    res.send({nextPeriod : nextPeriod.toString() + " Days" ,"ovulationStart" : (nextPeriod+14).toString() + " Days" });
    return;
  }
...
app.post('/logger', async (req, res) => {
  const {email, date, symptom} = req.body;
  console.log(email, date, symptom)
  if(!email | !symptom | !date) {res.send("fill all details"); return}
  const client = await MongoClient.connect(url);
  const db = client.db(dbName);
  const existingUser = await db.collection('users').findOne({ email });
  if (existingUser) {
    await db.collection('logHistory').insertOne(req.body);
    res.send("logged symptom");
    return;
  }
  res.send({message: "user not found"})
})
...
app.get('/logger/:email', async (req, res) => {
  const {email} = req.params;
  console.log(email + " - getting log history") 
  const client = await MongoClient.connect(url);
  const db = client.db(dbName);
  const existingUser = await db.collection('users').findOne({ email })
  if (existingUser) {
    list1 = await db.collection('logHistory').find().toArray(function(err,arr){
      return arr;
   });
   list1 = list1.filter(x => {return x.email == email})
    console.log(list1)
    if(list1.length == 0){
      res.send({message : "no logs found"})
    }
    else{
      res.send(list1)
    }
...
app.post('/question', async (req, res) => {
  const {email, hastag, question} = req.body;
  console.log(email, hastag, question)
  if(!email | !hastag | !question) {res.send("fill all details"); return}
  const client = await MongoClient.connect(url);
  const db = client.db(dbName);
  const existingUser = await db.collection('users').findOne({ email });
  if (existingUser) {
    await db.collection('feed').insertOne(req.body);
    res.send("post saved");
    return;
  }
  res.send({message: "user not found"})
...
app.get('/getUser/:user', async (req, res) => {
  ...
  console.log(user)
  if(!user) {res.send("no user in params"); return}
  const client = await MongoClient.connect(url);
  const db = client.db(dbName);
  const existingUser = await db.collection('users').findOne({ email:user });
  if (existingUser) {
